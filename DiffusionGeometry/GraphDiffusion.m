...
%      [XIsTransposed]     : Data matrix is points by dimension rather than dimensions times points. Default: 0.
%      [NoSelfLoop]        : Enforcing W(i,i)=0 to remove self-loops. It affects only the case Metric is not given.
%                           Not for computing e-ft and e-value, for PDE. Default: 0.
...
%               AutotuneDistances :   returned only if kNNAutotune specified, it's the vector of
%                                     distances of the kNNAutotune-th neighbor
...
for j=1:length(fn)
    ...
    elseif strcmpi(name,'Metric')           G.Opts.Metric = value;
    ...
    elseif strcmpi(name,'XIsTransposed')    G.Opts.XIsTransposed = value;
...
if ~isempty(Points)
    if G.Opts.XIsTransposed,    [N,Dim] = size(Points);
    else                        [Dim,N] = size(Points); end;
elseif lDistInfoProvided
...
if isempty(G.Opts.W),
    ...
    if isempty(DistInfo)
        ...
        lOptsTemp.XIsTransposed = G.Opts.XIsTransposed;
        if (G.Opts.NNMaxDim>0) && (G.Opts.NNMaxDim<Dim) && (G.Opts.NNMaxDim<N),
            if G.Opts.XIsTransposed,
                lPointsProj = Points';
            else
                lPointsProj = Points;
            end;
            lPointsProj = bsxfun(@minus,lPointsProj,mean(lPointsProj,2));
            [U,~,~] = randPCA(lPointsProj,G.Opts.NNMaxDim);
            lPointsProj = U'*lPointsProj;
            if G.Opts.XIsTransposed,
                lPointsProj = lPointsProj';
            end;
            PointsOld = Points;
            Points = lPointsProj;
        end;
        ...
        if G.Opts.DoRadiusSearch,       % Do radius search
            if ~exist('Metric')
                [DistInfo.count, DistInfo.idxs, DistInfo.dists] = nrsearch(Points, [], [], max([G.Opts.Radius,G.Opts.PCARadius]), [], lOptsTemp);
            else
                [DistInfo.count, DistInfo.idxs, DistInfo.dists] = nrsearch(Points, [], [], max([G.Opts.Radius,G.Opts.PCARadius]), Metric, lOptsTemp);
            end;
        else                            % Do nearest neighbors search
            if ~exist('Metric')
                [DistInfo.count, DistInfo.idxs, DistInfo.dists] = nrsearch(Points, [], max(G.Opts.kNN), [], [], lOptsTemp);
            else
                [DistInfo.count, DistInfo.idxs, DistInfo.dists] = nrsearch(Points, [], max(G.Opts.kNN), [], G.Opts.Metric, lOptsTemp);
            end;
